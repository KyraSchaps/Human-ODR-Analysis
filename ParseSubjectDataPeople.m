function [ data ] = ParseSubjectDataPeople( subject, pupil,plotit)
...
for jj=1:length(files);  %For all the files
    ...
    if len > 20
                holding=[statusData.sinceChangePT]';
        holding2=zeros(size(holding));
        holding2(2:length(holding)+1)=holding;
        holding2=holding2(1:end-1);
        
        futuremean=[statusData.distMean];
        currentmean=[futuremean(1,1),futuremean(1:end-1)]';
...
%         
      for i=1:length([statusData.isGoodTrial])
           ...
           if statusData(i).isSampled
%            disp(i);
               ProbeSamples=[statusData(1:i).targetAngle];  % If only want samples since last Probe Trial: statusData(i).targPerSample;
           [Beleif,Relevance,Reliability]=CPPandRelia2(ProbeSamples,statusData(i).H,statusData(i).distSTD);
           subject_data(i,12)=Beleif;
           subject_data(i,13)=Relevance;
           subject_data(i,14)=Reliability;
           end
        end
%        
...
AllPupilDataOfInt={};
for fff=1:length(wmfiles)
    subject2=wmfiles(fff);
    ToZeeScore=[];
    Testing=[];
    forColumnAvg=[];
           DATA(1,subject2).DelayPupilData={};
        
  
            DATA(1,subject2).DelayStartTimes= data{strcmp(data{:,1},fileNAMES{subject2-2}) ,10}'; 
            
            [C,B,DelayIndexes]=intersect(DATA(1,subject2).DelayStartTimes,DATA(1,subject2).time);
            ...
            for j=1:length(DelayIndexes)
               if nnz(isnan(DATA(subject2).missingSamp(DelayIndexes(j)-1000:DelayIndexes(j)+3000)))<.3*4000
                ...
                ToZeeScore=[ToZeeScore,DATA(1,subject2).DelayPupilData{j,1}];
            end
...
%             end
        ...
        AllPupilDataOfInt=[AllPupilDataOfInt;(DATA(1,subject2).DelayPupilData)];
